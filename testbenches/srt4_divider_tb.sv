`timescale 1ns/10ps

module srt4_divider_tb;

localparam W      = 32;
localparam period = 10;

logic [W-1:0] x, y, q, r;
logic         signed_in, start, clk, reset;
logic         done, busy, div_zero;
logic         passed;

srt4_divider #(.WORD_LENGTH(W)) dut (
    .clk       (clk),
    .reset     (reset),
    .start     (start),
    .signed_in (signed_in),
    .x         (x),
    .y         (y),
    .q         (q),
    .r         (r),
    .busy      (busy),
    .done      (done),
    .div_zero  (div_zero)
);

always #(period / 2) clk = ~clk;

task automatic run(
    input logic [W-1:0] xv, yv,
    input logic         is_signed,
    input logic [W-1:0] eq, er,
    input logic         expect_dz,
    input string        name
);
    x = xv; y = yv; signed_in = is_signed;
    reset = 1'b1;
    start = 1'b0;
    #period;
    reset = 1'b0;
    start = 1'b1;
    #period;
    start = 1'b0;
    wait (done);
    #period;

    if (expect_dz) begin
        if (div_zero !== 1'b1) begin
            passed = 0;
            $display("FAIL [%s]  div_zero not raised  x=%0d y=%0d  got div_zero=%b",
                     name, xv, yv, div_zero);
        end else
            $display("PASS [%s]  div_zero raised", name);
    end else begin
        if (q !== eq || r !== er) begin
            passed = 0;
            if (is_signed)
                $display("FAIL [%s]  x=%0d y=%0d  got q=%0d r=%0d  exp q=%0d r=%0d",
                         name, $signed(xv), $signed(yv),
                         $signed(q), $signed(r), $signed(eq), $signed(er));
            else
                $display("FAIL [%s]  x=%0d y=%0d  got q=%0d r=%0d  exp q=%0d r=%0d",
                         name, xv, yv, q, r, eq, er);
        end else begin
            if (is_signed)
                $display("PASS [%s]  %0d / %0d => q=%0d r=%0d",
                         name, $signed(xv), $signed(yv), $signed(q), $signed(r));
            else
                $display("PASS [%s]  %0d / %0d => q=%0d r=%0d",
                         name, xv, yv, q, r);
        end
    end
endtask

// Unsigned expected result
function automatic void uexp(
    input  logic [W-1:0] xv, yv,
    output logic [W-1:0] eq, er
);
    eq = xv / yv;
    er = xv % yv;
endfunction

// Signed expected result — truncated (round-toward-zero) division.
// Quotient sign: negative iff operands have opposite signs.
// Remainder sign: matches dividend (C99 / IEEE semantics).
function automatic void sexp(
    input  logic [W-1:0] xv, yv,
    output logic [W-1:0] eq, er
);
    logic signed [W-1:0] xs, ys;
    logic        [W-1:0] xa, ya;
    logic        [W-1:0] mq, mr;
    logic                neg_q, neg_r;

    xs = $signed(xv);
    ys = $signed(yv);

    neg_q = (xs[W-1] ^ ys[W-1]);
    neg_r =  xs[W-1];

    xa = xs[W-1] ? W'(-xs) : W'(xs);
    ya = ys[W-1] ? W'(-ys) : W'(ys);

    mq = xa / ya;
    mr = xa % ya;

    eq = neg_q ? W'(-$signed(mq)) : mq;
    er = neg_r ? W'(-$signed(mr)) : mr;
endfunction

initial begin
    clk    = 0;
    passed = 1;
    reset  = 0;
    start  = 0;

    // ============================================================
    // UNSIGNED
    // ============================================================
    $display("\n=== UNSIGNED: basic ===");
    run(0,   5,  0,  0,  0,  0, "0/5");
    run(1,   1,  0,  1,  0,  0, "1/1");
    run(10,  3,  0,  3,  1,  0, "10/3");
    run(100, 7,  0,  14, 2,  0, "100/7");
    run(17,  17, 0,  1,  0,  0, "17/17");
    run(16,  4,  0,  4,  0,  0, "16/4");
    run(15,  16, 0,  0,  15, 0, "15/16");

    $display("\n=== UNSIGNED: divide by 1 ===");
    run(0,             1, 0, 0,             0, 0, "0/1");
    run(1,             1, 0, 1,             0, 0, "1/1 again");
    run(32'hFFFF_FFFF, 1, 0, 32'hFFFF_FFFF, 0, 0, "MAXU/1");

    $display("\n=== UNSIGNED: large values ===");
    run(32'hFFFF_FFFF, 32'd2,         0, 32'h7FFF_FFFF, 32'd1,         0, "MAXU/2");
    run(32'hFFFF_FFFF, 32'hFFFF_FFFF, 0, 32'd1,         32'd0,         0, "MAXU/MAXU");
    run(32'hFFFF_FFFE, 32'hFFFF_FFFF, 0, 32'd0,         32'hFFFF_FFFE, 0, "(MAXU-1)/MAXU");
    run(32'h8000_0000, 32'd3,         0, 32'h2AAA_AAAA, 32'd2,         0, "2^31/3");

    $display("\n=== UNSIGNED: powers of 2 ===");
    for (int i = 1; i <= 31; i++) begin
        automatic logic [W-1:0] denom = 1 << i;
        automatic logic [W-1:0] eq2, er2;
        uexp(32'hFFFF_FFFF, denom, eq2, er2);
        run(32'hFFFF_FFFF, denom, 0, eq2, er2, 0, $sformatf("MAXU/2^%0d", i));
    end

    $display("\n=== UNSIGNED: divide by zero ===");
    run(42, 0, 0, 'x, 'x, 1, "42/0");
    run(0,  0, 0, 'x, 'x, 1, "0/0");

    $display("\n=== UNSIGNED: random (100 cases) ===");
    begin
        logic [W-1:0] rx, ry, eq2, er2;
        for (int i = 0; i < 100; i++) begin
            rx = $urandom();
            ry = $urandom_range(1, 32'hFFFF_FFFF);
            uexp(rx, ry, eq2, er2);
            run(rx, ry, 0, eq2, er2, 0, $sformatf("rand_u_%0d", i));
        end
    end

    // ============================================================
    // SIGNED
    // ============================================================
    $display("\n=== SIGNED: basic ===");
    begin
        logic [W-1:0] eq2, er2;
        sexp( 10,  3, eq2, er2); run( 10,  3, 1, eq2, er2, 0, "+10/+3");
        sexp(-10,  3, eq2, er2); run(-10,  3, 1, eq2, er2, 0, "-10/+3");
        sexp( 10, -3, eq2, er2); run( 10, -3, 1, eq2, er2, 0, "+10/-3");
        sexp(-10, -3, eq2, er2); run(-10, -3, 1, eq2, er2, 0, "-10/-3");
        sexp(  7,  2, eq2, er2); run(  7,  2, 1, eq2, er2, 0, "+7/+2");
        sexp( -7,  2, eq2, er2); run( -7,  2, 1, eq2, er2, 0, "-7/+2");
        sexp(  0,  5, eq2, er2); run(  0,  5, 1, eq2, er2, 0, "0/5");
    end

    $display("\n=== SIGNED: boundary values ===");
    begin
        logic [W-1:0] eq2, er2;
        sexp(32'h7FFF_FFFF, 32'h0000_0001, eq2, er2); run(32'h7FFF_FFFF, 32'h0000_0001, 1, eq2, er2, 0, "INT_MAX/1");
        sexp(32'h8000_0000, 32'h0000_0002, eq2, er2); run(32'h8000_0000, 32'h0000_0002, 1, eq2, er2, 0, "INT_MIN/2");
        sexp(32'h7FFF_FFFF, 32'hFFFF_FFFF, eq2, er2); run(32'h7FFF_FFFF, 32'hFFFF_FFFF, 1, eq2, er2, 0, "INT_MAX/-1");
        sexp(32'h7FFF_FFFF, 32'h7FFF_FFFF, eq2, er2); run(32'h7FFF_FFFF, 32'h7FFF_FFFF, 1, eq2, er2, 0, "INT_MAX/INT_MAX");
        sexp(32'h8000_0001, 32'h7FFF_FFFF, eq2, er2); run(32'h8000_0001, 32'h7FFF_FFFF, 1, eq2, er2, 0, "(INT_MIN+1)/INT_MAX");
    end

    $display("\n=== SIGNED: divide by 1 and -1 ===");
    begin
        logic [W-1:0] eq2, er2;
        sexp( 99,  1, eq2, er2); run( 99,  1, 1, eq2, er2, 0, "+99/+1");
        sexp(-99,  1, eq2, er2); run(-99,  1, 1, eq2, er2, 0, "-99/+1");
        sexp( 99, -1, eq2, er2); run( 99, -1, 1, eq2, er2, 0, "+99/-1");
        sexp(-99, -1, eq2, er2); run(-99, -1, 1, eq2, er2, 0, "-99/-1");
    end

    $display("\n=== SIGNED: divide by zero ===");
    run(-1, 0, 1, 'x, 'x, 1, "-1/0");
    run( 0, 0, 1, 'x, 'x, 1,  "0/0 signed");

    $display("\n=== SIGNED: random (100 cases) ===");
    begin
        logic [W-1:0] rx, ry, eq2, er2;
        for (int i = 0; i < 100; i++) begin
            rx = $urandom();
            do ry = $urandom(); while (ry == 0);
            // avoid INT_MIN / -1 overflow
            if (rx == 32'h8000_0000 && ry == 32'hFFFF_FFFF) ry = 2;
            sexp(rx, ry, eq2, er2);
            run(rx, ry, 1, eq2, er2, 0, $sformatf("rand_s_%0d", i));
        end
    end

    $display("\n========================================");
    if (passed)
        $display(" ALL TESTS PASSED");
    else
        $display(" *** FAILURES DETECTED ***");
    $display("========================================");
    $stop;
end

initial begin
    #(period * (W * 4 + 20) * 500);
    $display("WATCHDOG: simulation hung, aborting.");
    $stop;
end

endmodule