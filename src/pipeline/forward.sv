module forward # (
    ...
    assign fwd_rr1 = (
        (mem_rf_write && (ex_rs1 == mem_rd)) ? mem_alu :
        (wb_rf_write && (ex_rs1 == wb_rd)) ? wb_alu : ex_rr1
    );

    assign fwd_rr2 = (
        (mem_rf_write && (ex_rs2 == mem_rd)) ? mem_alu :
        (wb_rf_write && (ex_rs2 == wb_rd)) ? wb_alu : ex_rr2
    );
endmodule